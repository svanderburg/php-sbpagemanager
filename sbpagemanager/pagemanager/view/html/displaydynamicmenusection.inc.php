<?php
require_once(dirname(__FILE__)."/../../model/entities/PageEntity.class.php");
	...
	if($level <= count($query))
	{
		...
		if($checker->checkWritePermissions())
		{
			if($parentId !== "")
				$parentId = "/".$parentId;
			?>
			<a class="create_page" href="<?php print($_SERVER["SCRIPT_NAME"].$parentId); ?>?__operation=create_page">Create page</a>
			<?php
		}
	}
}
